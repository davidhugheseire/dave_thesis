...
view: away_win_training_input {
  derived_table: {
    explore_source: games {
      ...
      column: away_win                        { field: game_stats.away_win }
      filters: {
        field: games.season
...
view: away_win_testing_input {
  derived_table: {
    explore_source: games {
      ...
      filters: {
        field: games.season
        value: "2009,2011,2012,2013,2014,2015,2018"
      }
    }
  }
}
...
view: away_win_model {
  derived_table: {
    ...
    sql_create:
      ...
      SELECT
         * EXCEPT(id)
      FROM ${away_win_training_input.SQL_TABLE_NAME};;
  }
}
...
view: away_win_model_evaluation {
  derived_table: {
    sql: SELECT * FROM ml.EVALUATE(
          MODEL ${away_win_model.SQL_TABLE_NAME},
          (SELECT * FROM ${away_win_testing_input.SQL_TABLE_NAME}));;
  }
...
#
#
view: away_win_model_training_info {
  derived_table: {
    sql: SELECT  * FROM ml.TRAINING_INFO(MODEL ${away_win_model.SQL_TABLE_NAME});;
  }
...
view: away_win_prediction {
  derived_table: {
    sql: SELECT * FROM ml.PREDICT(
          MODEL ${away_win_model.SQL_TABLE_NAME},
          (SELECT * FROM ${away_win_future_input.SQL_TABLE_NAME}));;
  }
...
#     sql:
#     CASE
#     WHEN predicted_win = 1 THEN 'Home Win'
#     ELSE 'Draw or Away Win'
#     END  ;;
#   }
...
#       when: {
#         sql: predicted_win = 1 ;;
#         label: "Looker Predicts Home Win"}
#       when: {
#         sql: predicted_win = 2 ;;
#         label: "Looker Predicts Away Win"}
#       when: {
#         sql: predicted_win = 3 ;;
